...
- `com.ironhub.IronHubPlugin` — entry point; a `@Provides Set<IronHubModule>` registers 16 of 21 modules (RuneLite ships Guice without the multibindings extension). TODOs mark the rest: skills, QoL, loot/supplies, boat. Config toggles gate module lifecycles (`enabled()` + ConfigChanged sync).
...
- `com.ironhub.modules.*` — one package per module. **Implemented (M3): quests, diaries, ca** — read-only trackers built entirely from documented API constants (`Quest` API, `Varbits.DIARY_*`, `VarbitID.CA_*`); modules contribute tabs via `IronHubModule.buildTab()`, routed by nav-row name. **M4 in progress: bank** — snapshot search with persisted item names + sprite icons (frame 2f search section); banked XP / QoL / dailies detection still pending. The rest are stubs with Javadoc pointing at their DESIGN.md section.
